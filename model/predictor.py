...
class LitigationPredictor:
    ...
    Expected Value Formula (gewichtetes Mixture zweier Verteilungen):
        ...
        Jurist-Verteilung : P_jur  = (win=juristic, loss=1-juristic)
        ML-Verteilung     : P_ml   = (win=p_win_ml, loss=p_loss_ml)
    ...
    def predict(self, case_dict: dict) -> dict:
        ...
        return {
            ...
            "probabilities": {
                OUTCOME_LABELS[i]: float(probs_np[i]) for i in range(2)
            },
            "confidence": confidence,
            "p_win": float(probs_np[1]),
    ...
    def compute_expected_value(
        ...
        p_failure = max(0.0, 1.0 - p_full_success)

        ev_probability = p_full_success
